/**
 * Migration 078: Per-user OAuth on downstream MCP connections
 *
 * Adds `auth_mode` to `connections` and re-introduces `userId` on
 * `downstream_tokens` as a nullable column. PR #2203 (migration 017) had
 * removed `userId` because all tokens were treated as connection-scoped.
 *
 ...
 *   - `downstream_tokens_shared_unique` on (connection_id) WHERE userId IS NULL
 *   - `downstream_tokens_per_user_unique` on (connection_id, userId)
